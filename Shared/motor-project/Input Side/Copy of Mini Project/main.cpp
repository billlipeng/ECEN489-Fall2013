			...
			while(working)
			{
				if(canPrint)
				{
					...
					if(reading.GetErrorMessage() == "")
					{
						...
						stringstream stringStream;
						stringStream << reading.GetAccelerationXAxis()<< " "<<reading.GetAccelerationYAxis()<<" "<<reading.GetAccelerationZAxis();
						...
						string sqlCommand = "INSERT INTO data_collection_test";
												sqlCommand += "(acceleration_x, acceleration_y, acceleration_z)";
												sqlCommand += " VALUES (";
