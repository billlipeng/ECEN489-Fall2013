		...
		if (readResult > 0)
		{
			...
			while(working)
			{
				if(canPrint)
				{
					...
					/*
					 cmd = strcat(q, data_values[0]);

					cmd = strcat(cmd, ",");
					cmd = strcat(cmd, data_values[1]);
					cmd = strcat(cmd, ")");
					*/
					...
					//const char *command = "INSERT INTO temp_readings"
					//		"(arduino_id, temp_kelvin) VALUES (" + data_values[0] + "," + data_values[1] + ")";
					/*PGresult *op_result = PQexecParams(
							conn,
							command,
							2,
							NULL,
...
{
	canPrint = true;
	const int numberOfArduinos = 1;
	int comPorts [numberOfArduinos]= {12};
