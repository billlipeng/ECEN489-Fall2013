...
#define MAGN_X_MAX ((float) 625)
#define MAGN_Y_MIN ((float) -470)
#define MAGN_Y_MAX ((float) 503)
#define MAGN_Z_MIN ((float) -471)
...
#define GYRO_AVERAGE_OFFSET_X ((float) -3.50)
#define GYRO_AVERAGE_OFFSET_Y ((float) 3.40)
#define GYRO_AVERAGE_OFFSET_Z ((float) 11.30)


// Calibration variables
float sensorZero[6]= {1.48, 8.48, -45.68, -3.50, 3.40, 11.30};  // calibration values
//float sensorZero[6];
...
//#define GYRO_SCALED_DEG(x) (x * TO_DEG(GYRO_GAIN)) // Calculate the scaled gyro readings in degree per second
...
#define TO_METERS_PER_SEC2_X(x) (x*4*pow(10,-3)*9.80665) // in uint: m/s2
#define TO_METERS_PER_SEC2_Y(x) (x*4*pow(10,-3)*9.80665) // in uint: m/s2
#define TO_METERS_PER_SEC2_Z(x) (x*4*pow(10,-3)*9.80665) // in uint: m/s2
...
// test modification
#define BACK_FORWARD 0 
#define UP_DOWN  1

boolean set_test_mode;

// Sensor variables
float accel[3];  // Actually stores the NEGATED acceleration (equals gravity, if board not moving).
float accel_min[3];
float accel_max[3];
float accel_x_in_global_frame;
float accel_y_in_global_frame;
float accel_z_in_global_frame;
...
float x_acceleration[2], y_acceleration[2], z_acceleration[2];
float x_velocity[2], y_velocity[2], z_velocity[2];
float x_position[2], y_position[2], z_position[2];

// double ss_gyro_x, ss_accel_x, ss_accel_y, ss_accel_z; 
float countx_gyro, countx_accel, county_accel, countz_accel;
...
void noise_reduction(float RawAccX, float RawAccY, float RawAccZ){
  gXFilt = ( 1-alpha) * gXFilt + alpha * RawAccX;
  gYFilt = ( 1-alpha) * gYFilt + alpha * RawAccY;
  gZFilt = ( 1-alpha) * gZFilt + alpha * RawAccZ; 
  gFilt[0] = gZFilt;
  gFilt[1] = gYFilt;
  gFilt[2] = gXFilt;
}
...
void loop()
{
  ...
  if ( Serial.available() != 0)
  {
     ...
     if (cmd == '0')
	 {
            Serial.println("BACK_FORWARD");
	    set_test_mode = BACK_FORWARD;
	 }
	 
     else if ( cmd == '1')
	 {
            Serial.println("UP_DOWN");
	    set_test_mode = UP_DOWN;
	 }
  }
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    else if (output_mode == OUTPUT__MODE_ANGLES)  // Output angles
    {
      ...
      Euler_angles();
      Acc_In_Global_Frame();
      noise_reduction(accel_x_in_global_frame, accel_y_in_global_frame, accel_z_in_global_frame);
      ACC_angle = getAccAngle(ACC_Z);                                                 // in Quids
      GYRO_rateX = getGyroRate(GYR_X);   
      //GYRO_rateY = getGyroRate(GYR_Y);     // in Quids/seconds
      actAngleX = kalmanCalculate(ACC_angle, GYRO_rateX, lastLoopTime); 
      if (output_stream_on || output_single_on) {
        output_acc_in_global_frame();
        //output_angles();
