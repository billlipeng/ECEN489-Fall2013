...
void output_acc_in_global_frame()
{
   if (output_format == OUTPUT__FORMAT_TEXT)
  {
    if( set_mode == UP_DOWN)
    {
     ...
     gFilt[1] = gFilt[1]/2 + 1; 
     gFilt[2] = gFilt[2]/2 - 1;
     if( gFilt[0] < 2 && gFilt[0] > -2)
       gFilt[0] = 0;
     if( gFilt[1] < 2 && gFilt[1] > -2)
       gFilt[1] = 0;
     if( gFilt[2] < 2 && gFilt[2] > -2)
       gFilt[2] = 0;
       
   // Serial.println("hi");
    Serial.print(gFilt[2]);  //X axis filtered
    Serial.print(",");
    Serial.print(gFilt[1]);  //Y axis filtered
    Serial.print(",");
    Serial.print(gFilt[0]);  //Z axis filtered
    Serial.println();
   // Serial.print(","); 
   // Serial.println("up_down"); 
    }
     
    else if( set_mode == BACK_FORWARD)
    {
     gFilt[0] = gFilt[0]/2 - 7;
     gFilt[1] = gFilt[1]/2 + 1; 
     gFilt[2] = gFilt[2]/2 - 1;
    ...
    Serial.print(gFilt[0]);  //X axis filtered
    Serial.print(",");
    Serial.print(gFilt[1]);  //Y axis filtered
    Serial.print(",");
    Serial.print(gFilt[2]);  //Z axis filtered
    Serial.println();
   // Serial.print(","); 
...
void  rotation_angle(){
  ...
  if (output_format == OUTPUT__FORMAT_TEXT){
    if( set_mode == BACK_FORWARD)
    {
     float angle = 60 - actAngleX;
     if( angle < 30) angle = 0;
     Serial.print(angle);
     Serial.print(","); 
    ...
    else if( set_mode == UP_DOWN)
     Serial.println(actAngleX);
