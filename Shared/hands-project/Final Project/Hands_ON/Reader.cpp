...
void* RunThread(void* threadArgument)
{
    ...
    while(true)//reader->GetSerial()->IsConnected())
    {
        //system("cls");
        char incomingData[dataLength];

        if (reader->GetSerial()->IsConnected())
        {
    ...
    //            }
        ...
        else
        {
            ...
            cout<<"had to reset"<<reader->GetComPortNumber()<<endl;

            string comPortNumberString = "";
            stringstream stringStream;
            stringStream << reader->GetComPortNumber();
            stringStream>>comPortNumberString;
            string comPortString = "\\\\.\\COM" + comPortNumberString;
            ...
            Serial* serial = reader->GetSerial();
            serial = new Serial(comPortPointer);
        }
...
double Reader::GetPingDistance()
{
    Reading reading = _reading;
    double distance = reading.GetPingDistance(_previousDistance);
    _previousDistance = reading.GetPreviousDistance();
    return distance;
}
